...
const PostCreateModal = ({ isOpen, close }) => {
  ...
  const [tags, setTags] = useState(''); // tag도 useState로 하면 되는 게 맞나....?
  ...
  const handleImgUpload = async (e) => {
    ...
    const { data: uploadData, error: uploadError } = await supabase.storage
      .from('my-bucket-name')
      .upload(fileName, postImgFile);
    // 에러 발생 시 실행될 기능
    if (uploadError) {
      console.log('포스트 이미지 파일 업로드 중 에러 발생 ==> ', uploadError);
      return;
    }
    // 업로드한 이미지의 public URL 가져오기
    const { publicURL, error: urlError } = supabase.storage.from('my-bucket-name').getPublicUrl(fileName); // 이전에 만들어둔 my-bucket-name이름의 버켓에서 이미지 파일의 publicUrl을 가져온다
    // 파일 url 가져오기 중 에러 발생 시 실행
    if (urlError) {
      console.log('포스트 이미지 파일 url 가져오기 중 에러 발생 ==> ', urlError);
      return;
    }
    ...
    // 에러 발생 시 실행될 기능
    if (postError) {
      console.log('posts테이블에 이미지 url추가 중 에러 발생 ==> ', postError);
    } else {
  ...
  const handleAddPost = async (e) => {
    ...
    const { data: sessionData } = await supabase.auth.getSession(); // supabase에서 세션정보(현재 로그인한 유저의 정보)를 받아와서 그 데이터를 sessionData라는 이름의 변수로 저장
    const user = sessionData?.session?.user; // getSession에 있는 data라는 객체안에 있는 session이라 객체안에서 user라는 키워드의 값을 가져와 user라는 이름의 변수로 지정 / ?. <== 이거는 Optional Chaining란 것으로 속성이 없을 떄 undefined를 반환해 오류가 발생하지 않게 함!
    ...
    setPosts((prev) => [
      ...
      {
        id: data[0].id, // id는 supabase가 자동으로 만들어준다!
        ...
        <StForm onSubmit={handleSubmit}>
          ...
          <StInputWrapper>
            ...
            <StTextInputWrapper>
              ...
              <StLabel>
                내용
                <textarea required onChange={(e) => setContents(e.target.value)}></textarea>
              </StLabel>
              <StLabel>
                태그
                <input type="text" required onChange={(e) => setTags(e.target.value)} />
              </StLabel>
            </StTextInputWrapper>
          </StInputWrapper>
